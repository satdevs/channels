...
<?php 
	...
	if(!isset($layoutLanguagesLastId)){
		$layoutLanguagesLastId = 0;
	}
...
?>
		<div class="index col-12">
            <div class="card card-lightblue">
				...
				<?php //= __('Languages') ?>	
				<div class="card-body table-responsive p-0 languages">
...
<?php if(isset($config['index_show_counts']) && $config['index_show_counts']){ ?>
								<th class="program-count integer"><?= $this->Paginator->sort('program_count') ?></th>
<?php } ?>
...
<?php } ?>
					...
					<?php foreach ($languages as $language): ?>
							<tr row-id="<?= $language->id ?>"<?php if($language->id == $layoutLanguagesLastId){ echo ' class="table-tr-last-id"'; } ?>  prefix="<?= $prefix ?>" controller="<?= $controller ?>" action="<?= $action ?>" aria-expanded="true">
								<td class="row-id-anchor" value="<?= $language->id ?>"><a class="anchor" name="<?= $language->id ?>"></a></td><!-- bake-0 -->
<?php if(isset($config['index_show_id']) && $config['index_show_id']){ ?>
								<td class="id integer" name="id" value="<?= $this->Number->format($language->id) ?>"><?= $this->Number->format($language->id) ?></td><!-- bake-3 -->
<?php } ?>
								<td class="name string" name="name" value="<?= $language->name ?>"><?= h($language->name) ?></td><!-- bake-2 -->
<?php if(isset($config['index_show_visible']) && $config['index_show_visible']){ ?>
								<td class="visible boolean" name="visible" value="<?= $language->visible ?>"><?= h($language->visible) ?></td><!-- bake-2 -->
<?php } ?><?php if(isset($config['index_show_pos']) && $config['index_show_pos']){ ?>
								<td class="pos integer" name="pos" value="<?= $this->Number->format($language->pos) ?>"><?= $this->Number->format($language->pos) ?></td><!-- bake-3 -->
<?php } ?>
<?php if(isset($config['index_show_counts']) && $config['index_show_counts']){ ?>
								<td class="program-count integer" name="program-count" value="<?= $this->Number->format($language->program_count) ?>"><?= $this->Number->format($language->program_count) ?></td><!-- bake-3 -->
<?php } ?>
...
<?php 	if(isset($config['index_enable_view']) && $config['index_enable_view']){ ?>					  
									<?= $this->Html->link('<i class="fas fa-eye"></i>', ['action' => 'view', $language->id], ['escape' => false, 'class' => 'btn btn-sm bg-gradient-warning action-button-view', 'data-bs-tooltip'=>'tooltip', 'data-bs-placement'=>'top', 'title' => __('View this record')]) ?>
<?php 	} ?>
<?php 	if(isset($config['index_enable_edit']) && $config['index_enable_edit']){ ?>					  
									<?= $this->Html->link('<i class="fas fa-edit"></i>', ['action' => 'edit', $language->id], ['escape' => false, 'class' => 'btn btn-sm bg-gradient-success action-button-edit', 'data-bs-tooltip'=>'tooltip', 'data-bs-placement'=>'top', 'title' => __('Edit this record')]) ?>
<?php 	} ?>			
<?php 	if($language->program_count == 0 && isset($config['index_enable_delete']) && $config['index_enable_delete']){ ?>					  
									<?php //= $this->Form->postLink('<i class="fas fa-remove"></i>', ['action' => 'delete', $language->id], ['escape' => false, 'confirm' => __('Are you sure you want to delete # {0}?', $language->id), 'class' => 'btn btn-sm bg-gradient-danger action-button-delete']) ?>						
									<?= $this->Form->postLink('', ['action' => 'delete', $language->id], ['class'=>'crose-btn hide-postlink action-button-delete']) ?>
									<a href="javascript:;" class="btn btn-sm btn-danger delete postlink-delete" data-bs-tooltip="tooltip" data-bs-placement="top" title="<?= __("Delete this record!") ?>" text="<?= h($language->name) ?>" subText="<?= __("You will not be able to revert this!") ?>" confirmButtonText="<?= __("Yes, delete it!") ?>" cancelButtonText="<?= __("Cancel") ?>"><i class="icon-minus"></i></a>
...
<?php $url = $this->Url->build(['controller' => 'Languages', 'action' => $config['index_db_click_action']]); ?>
		$('tr').dblclick( function(){
<?php /* window.location.href = '/<?= $prefix ?>/languages/<?= $config['index_db_click_action'] ?>/'+$(this).attr('row-id'); */ ?>
...
<?php if(isset($controller)){ ?>
		$(".pagination a[href^='<?= $base ?>/<?= $prefix ?>/languages?sort=']").each(function(){ 
			this.href = this.href.replace("<?= $base ?>/<?= $prefix ?>/languages?sort=", "<?= $base ?>/<?= $prefix ?>/languages?page=1&sort=");
		});
		$(".pagination a[href='<?= $base ?>/<?= $prefix ?>/languages']").each(function(){ 
			this.href = this.href.replace("<?= $base ?>/<?= $prefix ?>/languages", "<?= $base ?>/<?= $prefix ?>/languages?page=1");
		});
<?php } ?>
