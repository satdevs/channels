 ...
 * @var \App\Model\Entity\Headstation[]|\Cake\Collection\CollectionInterface $headstations
 */
?>
<?php 
...
?>
		<div class="index col-12">
            <div class="card card-lightblue">
				...
				<?php //= __('Headstations') ?>	
				<div class="card-body table-responsive p-0 headstations">
...
<?php if(isset($config['index_show_counts']) && $config['index_show_counts']){ ?>
								<th class="package-count integer"><?= $this->Paginator->sort('package_count') ?></th>
<?php } ?>
<?php if(isset($config['index_show_counts']) && $config['index_show_counts']){ ?>
								<th class="city-count integer"><?= $this->Paginator->sort('city_count') ?></th>
<?php } ?>
...
<?php } ?>
					...
					<?php foreach ($headstations as $headstation): ?>
							<tr row-id="<?= $headstation->id ?>"<?php if($headstation->id == $layoutHeadstationsLastId){ echo ' class="table-tr-last-id"'; } ?>  prefix="<?= $prefix ?>" controller="<?= $controller ?>" action="<?= $action ?>" aria-expanded="true">
								<td class="row-id-anchor" value="<?= $headstation->id ?>"><a class="anchor" name="<?= $headstation->id ?>"></a></td><!-- bake-0 -->
<?php if(isset($config['index_show_id']) && $config['index_show_id']){ ?>
								<td class="id integer" name="id" value="<?= $this->Number->format($headstation->id) ?>"><?= $this->Number->format($headstation->id) ?></td><!-- bake-3 -->
<?php } ?>
								<td class="name string" name="name" value="<?= $headstation->name ?>"><?= h($headstation->name) ?></td><!-- bake-2 -->
								<td class="place string" name="place" value="<?= $headstation->place ?>"><?= h($headstation->place) ?></td><!-- bake-2 -->
<?php if(isset($config['index_show_counts']) && $config['index_show_counts']){ ?>
								<td class="package-count integer" name="package-count" value="<?= $this->Number->format($headstation->package_count) ?>"><?= $this->Number->format($headstation->package_count) ?></td><!-- bake-3 -->
<?php } ?>
<?php if(isset($config['index_show_counts']) && $config['index_show_counts']){ ?>
								<td class="city-count integer" name="city-count" value="<?= $this->Number->format($headstation->city_count) ?>"><?= $this->Number->format($headstation->city_count) ?></td><!-- bake-3 -->
<?php } ?>
<?php if(isset($config['index_show_visible']) && $config['index_show_visible']){ ?>
								<td class="visible boolean" name="visible" value="<?= $headstation->visible ?>"><?= h($headstation->visible) ?></td><!-- bake-2 -->
<?php } ?><?php if(isset($config['index_show_pos']) && $config['index_show_pos']){ ?>
								<td class="pos integer" name="pos" value="<?= $this->Number->format($headstation->pos) ?>"><?= $this->Number->format($headstation->pos) ?></td><!-- bake-3 -->
<?php } ?>
...
<?php 	if(isset($config['index_show_created']) && $config['index_show_created']){ ?>
									<?= h($headstation->created) ?>
<?php 	} ?>
<?php 	if(isset($config['index_show_created']) && $config['index_show_created'] && isset($config['index_show_modified']) && $config['index_show_modified']){ ?>
									<br>
<?php 	} ?>
<?php 	if(isset($config['index_show_modified']) && $config['index_show_modified']){ ?>
									<?= h($headstation->modified) ?>
<?php 	} ?>
								</td>
<?php } ?>
...
<?php 	if(isset($config['index_enable_view']) && $config['index_enable_view']){ ?>					  
									<?= $this->Html->link('<i class="fas fa-eye"></i>', ['action' => 'view', $headstation->id], ['escape' => false, 'class' => 'btn btn-sm bg-gradient-warning action-button-view', 'data-bs-tooltip'=>'tooltip', 'data-bs-placement'=>'top', 'title' => __('View this record')]) ?>
<?php 	} ?>
<?php 	if(isset($config['index_enable_edit']) && $config['index_enable_edit']){ ?>					  
									<?= $this->Html->link('<i class="fas fa-edit"></i>', ['action' => 'edit', $headstation->id], ['escape' => false, 'class' => 'btn btn-sm bg-gradient-success action-button-edit', 'data-bs-tooltip'=>'tooltip', 'data-bs-placement'=>'top', 'title' => __('Edit this record')]) ?>
<?php 	} ?>			
<?php 	if(isset($config['index_enable_delete']) && $config['index_enable_delete']){ ?>					  
									...
									<?= $this->Form->postLink('', ['action' => 'delete', $headstation->id], ['class'=>'crose-btn hide-postlink action-button-delete']) ?>
									<a href="javascript:;" class="btn btn-sm btn-danger delete postlink-delete" data-bs-tooltip="tooltip" data-bs-placement="top" title="<?= __("Delete this record!") ?>" text="<?= h($headstation->name) ?>" subText="<?= __("You will not be able to revert this!") ?>" confirmButtonText="<?= __("Yes, delete it!") ?>" cancelButtonText="<?= __("Cancel") ?>"><i class="icon-minus"></i></a>
...
<?php $url = $this->Url->build(['controller' => 'Headstations', 'action' => $config['index_db_click_action']]); ?>
		$('tr').dblclick( function(){
<?php /* window.location.href = '/<?= $prefix ?>/headstations/<?= $config['index_db_click_action'] ?>/'+$(this).attr('row-id'); */ ?>
...
<?php if(isset($controller)){ ?>
		$(".pagination a[href^='<?= $base ?>/<?= $prefix ?>/headstations?sort=']").each(function(){ 
			this.href = this.href.replace("<?= $base ?>/<?= $prefix ?>/headstations?sort=", "<?= $base ?>/<?= $prefix ?>/headstations?page=1&sort=");
		});
		$(".pagination a[href='<?= $base ?>/<?= $prefix ?>/headstations']").each(function(){ 
			this.href = this.href.replace("<?= $base ?>/<?= $prefix ?>/headstations", "<?= $base ?>/<?= $prefix ?>/headstations?page=1");
		});
<?php } ?>
