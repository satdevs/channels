 ...
 * @var \App\Model\Entity\Packagegroup[]|\Cake\Collection\CollectionInterface $packagegroups
 */
?>
<?php 
...
?>
		<div class="index col-12">
            <div class="card card-lightblue">
				...
				<?php //= __('Packagegroups') ?>	
				<div class="card-body table-responsive p-0 packagegroups">
...
<?php } ?>
						...
						<tbody>
					<?php foreach ($packagegroups as $packagegroup): ?>
							<tr row-id="<?= $packagegroup->id ?>"<?php if($packagegroup->id == $layoutPackagegroupsLastId){ echo ' class="table-tr-last-id"'; } ?>  prefix="<?= $prefix ?>" controller="<?= $controller ?>" action="<?= $action ?>" aria-expanded="true">
								<td class="row-id-anchor" value="<?= $packagegroup->id ?>"><a class="anchor" name="<?= $packagegroup->id ?>"></a></td><!-- bake-0 -->
<?php if(isset($config['index_show_id']) && $config['index_show_id']){ ?>
								<td class="id integer" name="id" value="<?= $this->Number->format($packagegroup->id) ?>"><?= $this->Number->format($packagegroup->id) ?></td><!-- bake-3 -->
<?php } ?>
<?php if(isset($config['index_show_pos']) && $config['index_show_pos']){ ?>
								<td class="pos integer" name="pos" value="<?= $this->Number->format($packagegroup->pos) ?>"><?= $this->Number->format($packagegroup->pos) ?></td><!-- bake-3 -->
<?php } ?>
								<td class="name string" name="name" value="<?= $packagegroup->name ?>"><?= h($packagegroup->name) ?></td><!-- bake-2 -->
<?php if(isset($config['index_show_visible']) && $config['index_show_visible']){ ?>
								<td class="visible boolean" name="visible" value="<?= $packagegroup->visible ?>"><?= h($packagegroup->visible) ?></td><!-- bake-2 -->
<?php } ?>

<?php if(isset($config['index_show_created']) && $config['index_show_created'] || isset($config['index_show_modified']) && $config['index_show_modified']){ ?>
								<td class="datetime created-modified">
<?php 	if(isset($config['index_show_created']) && $config['index_show_created']){ ?>
									<?= h($packagegroup->created) ?>
<?php 	} ?>
<?php 	if(isset($config['index_show_created']) && $config['index_show_created'] && isset($config['index_show_modified']) && $config['index_show_modified']){ ?>
									<br>
<?php 	} ?>
<?php 	if(isset($config['index_show_modified']) && $config['index_show_modified']){ ?>
									<?= h($packagegroup->modified) ?>
<?php 	} ?>
								</td>
<?php } ?>
...
<?php 	if(isset($config['index_enable_view']) && $config['index_enable_view']){ ?>					  
									<?= $this->Html->link('<i class="fas fa-eye"></i>', ['action' => 'view', $packagegroup->id], ['escape' => false, 'class' => 'btn btn-sm bg-gradient-warning action-button-view', 'data-bs-tooltip'=>'tooltip', 'data-bs-placement'=>'top', 'title' => __('View this record')]) ?>
<?php 	} ?>
<?php 	if(isset($config['index_enable_edit']) && $config['index_enable_edit']){ ?>					  
									<?= $this->Html->link('<i class="fas fa-edit"></i>', ['action' => 'edit', $packagegroup->id], ['escape' => false, 'class' => 'btn btn-sm bg-gradient-success action-button-edit', 'data-bs-tooltip'=>'tooltip', 'data-bs-placement'=>'top', 'title' => __('Edit this record')]) ?>
<?php 	} ?>			
<?php 	if(isset($config['index_enable_delete']) && $config['index_enable_delete']){ ?>					  
									<?php //= $this->Form->postLink('<i class="fas fa-remove"></i>', ['action' => 'delete', $packagegroup->id], ['escape' => false, 'confirm' => __('Are you sure you want to delete # {0}?', $packagegroup->id), 'class' => 'btn btn-sm bg-gradient-danger action-button-delete']) ?>						
									<?= $this->Form->postLink('', ['action' => 'delete', $packagegroup->id], ['class'=>'crose-btn hide-postlink action-button-delete']) ?>
									<a href="javascript:;" class="btn btn-sm btn-danger delete postlink-delete" data-bs-tooltip="tooltip" data-bs-placement="top" title="<?= __("Delete this record!") ?>" text="<?= h($packagegroup->name) ?>" subText="<?= __("You will not be able to revert this!") ?>" confirmButtonText="<?= __("Yes, delete it!") ?>" cancelButtonText="<?= __("Cancel") ?>"><i class="icon-minus"></i></a>
...
<?php $url = $this->Url->build(['controller' => 'Packagegroups', 'action' => $config['index_db_click_action']]); ?>
		$('tr').dblclick( function(){
<?php /* window.location.href = '/<?= $prefix ?>/packagegroups/<?= $config['index_db_click_action'] ?>/'+$(this).attr('row-id'); */ ?>
...
<?php if(isset($controller)){ ?>
		$(".pagination a[href^='<?= $base ?>/<?= $prefix ?>/packagegroups?sort=']").each(function(){ 
			this.href = this.href.replace("<?= $base ?>/<?= $prefix ?>/packagegroups?sort=", "<?= $base ?>/<?= $prefix ?>/packagegroups?page=1&sort=");
		});
		$(".pagination a[href='<?= $base ?>/<?= $prefix ?>/packagegroups']").each(function(){ 
			this.href = this.href.replace("<?= $base ?>/<?= $prefix ?>/packagegroups", "<?= $base ?>/<?= $prefix ?>/packagegroups?page=1");
		});
<?php } ?>
