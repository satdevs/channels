...
<?php //$format = $formats[$locale]; ?>
		<div class="add col-sm-10">
			<div class="card card-lightblue">
				...
				<!-- form start -->
				<?= $this->Form->create($band, ['id' => 'main-form', 'class'=>'form-horizontal']) ?>
					<!-- card-body -->
					<div class="card-body" style="opacity: 0;">
						...
						<div class="form-group row">
							...
							<div class="col-sm-9">
								<?= $this->Form->control('name', ['placeholder' => __('Pl.: VHF I.; VHF III.; UHF; ...'), 'type'=>'text', 'class'=>'form-control', 'label' => false, 'empty' => true, 'autofocus' => true, "required" => true]) ?>
							</div>
						</div>
						...
						<div class="form-group row">
							...
							<div class="col-sm-3 col-xs-9">
								<?= $this->Form->control('band', ['placeholder' => __('Pl.: C2, C3, S2, S3, UHF21, UHF22, ...'), 'type'=>'text', 'class'=>'form-control', 'label' => false, 'autofocus' => false, "required" => false]) ?>
							</div>
						</div>

						<?php //debug($band_types); die('xxx'); ?>

						<!-- 6. string -->
						<div class="form-group row">
							<label for="type" class="col-sm-2 col-form-label"><?= __('Type') ?>:</label>
							<div class="col-sm-3 col-xs-9">
								<?= $this->Form->control('type', ['options' => $band_types, 'type'=>'select', 'class'=>'form-control select', 'label' => false, 'autofocus' => false, "required" => false]) ?>
							</div>
						</div>
						...
						<div class="form-group row">
							<label for="bandwidth" class="col-sm-2 col-form-label"><?= __('Bandwidth') ?>:</label>
							<div class="input-group col-xs-12 col-sm-10 col-md-8 col-lg-4 col-xl-3">
								<?= $this->Form->control('bandwidth', ['type' => 'number', 'class' => 'form-control number', 'label' => false, 'templates'=>[ 'inputContainer' => '{{content}}', 'inputContainerError' => '{{content}}{{error}}'], "required" => false]) ?>
							</div>
						</div>
						...
						<div class="form-group row">
							...
							<div class="input-group col-xs-12 col-sm-10 col-md-8 col-lg-4 col-xl-3">
								<?= $this->Form->control('audio_frequency', ['type' => 'number', 'class' => 'form-control number', 'label' => false, 'templates'=>[ 'inputContainer' => '{{content}}', 'inputContainerError' => '{{content}}{{error}}'], "required" => false]) ?>
							</div>
						</div>
...
<?php if(isset($config['form_show_counts']) && $config['form_show_counts']){ ?>
						...
						<div class="form-group row">
							...
							<div class="input-group col-xs-12 col-sm-10 col-md-8 col-lg-4 col-xl-4">
								<div class="count"><?= $band->packages_programs_count ?></div>
							</div>
						</div>
<?php } ?>
