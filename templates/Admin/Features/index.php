...
<?php 
	...
	if(!isset($layoutFeaturesLastId)){
		$layoutFeaturesLastId = 0;
	}
...
?>
		<div class="index col-12">
            <div class="card card-lightblue">
				...
				<?php //= __('Features') ?>	
				<div class="card-body table-responsive p-0 features">
...
<?php } ?>
					...
					<?php foreach ($features as $feature): ?>
							<tr row-id="<?= $feature->id ?>"<?php if($feature->id == $layoutFeaturesLastId){ echo ' class="table-tr-last-id"'; } ?>  prefix="<?= $prefix ?>" controller="<?= $controller ?>" action="<?= $action ?>" aria-expanded="true">
								<td class="row-id-anchor" value="<?= $feature->id ?>"><a class="anchor" name="<?= $feature->id ?>"></a></td><!-- bake-0 -->
<?php if(isset($config['index_show_id']) && $config['index_show_id']){ ?>
								<td class="id integer" name="id" value="<?= $this->Number->format($feature->id) ?>"><?= $this->Number->format($feature->id) ?></td><!-- bake-3 -->
<?php } ?>
								<td class="name string" name="name" value="<?= $feature->name ?>">
									<b><?= h($feature->name) ?></b><br>
									[<span class="text-wrap"><?php
									foreach($feature->programs as $program){
										echo $program->name . ' • ';
									}
									?></span>]
...
<?php if(isset($config['index_show_counts']) && $config['index_show_counts']){ ?>
								<td class="program-count integer" name="program-count" value="<?= $this->Number->format($feature->program_count) ?>"><?= $this->Number->format($feature->program_count) ?></td><!-- bake-3 -->
<?php } ?>
<?php if(isset($config['index_show_visible']) && $config['index_show_visible']){ ?>
								<td class="visible boolean" name="visible" value="<?= $feature->visible ?>"><?= h($feature->visible) ?></td><!-- bake-2 -->
<?php } ?><?php if(isset($config['index_show_pos']) && $config['index_show_pos']){ ?>
								<td class="pos integer" name="pos" value="<?= $this->Number->format($feature->pos) ?>"><?= $this->Number->format($feature->pos) ?></td><!-- bake-3 -->
<?php } ?>
...
<?php 	if(isset($config['index_enable_view']) && $config['index_enable_view']){ ?>					  
									<?= $this->Html->link('<i class="fas fa-eye"></i>', ['action' => 'view', $feature->id], ['escape' => false, 'class' => 'btn btn-sm bg-gradient-warning action-button-view', 'data-bs-tooltip'=>'tooltip', 'data-bs-placement'=>'top', 'title' => __('View this record')]) ?>
<?php 	} ?>
<?php 	if(isset($config['index_enable_edit']) && $config['index_enable_edit']){ ?>					  
									<?= $this->Html->link('<i class="fas fa-edit"></i>', ['action' => 'edit', $feature->id], ['escape' => false, 'class' => 'btn btn-sm bg-gradient-success action-button-edit', 'data-bs-tooltip'=>'tooltip', 'data-bs-placement'=>'top', 'title' => __('Edit this record')]) ?>
<?php 	} ?>			
<?php 	if($feature->program_count == 0 && isset($config['index_enable_delete']) && $config['index_enable_delete']){ ?>

									<?php //= $this->Form->postLink('<i class="fas fa-remove"></i>', ['action' => 'delete', $feature->id], ['escape' => false, 'confirm' => __('Are you sure you want to delete # {0}?', $feature->id), 'class' => 'btn btn-sm bg-gradient-danger action-button-delete']) ?>						
									<?= $this->Form->postLink('', ['action' => 'delete', $feature->id], ['class'=>'crose-btn hide-postlink action-button-delete']) ?>
									<a href="javascript:;" class="btn btn-sm btn-danger delete postlink-delete" data-bs-tooltip="tooltip" data-bs-placement="top" title="<?= __("Delete this record!") ?>" text="<?= h($feature->name) ?>" subText="<?= __("You will not be able to revert this!") ?>" confirmButtonText="<?= __("Yes, delete it!") ?>" cancelButtonText="<?= __("Cancel") ?>"><i class="icon-minus"></i></a>
...
<?php $url = $this->Url->build(['controller' => 'Features', 'action' => $config['index_db_click_action']]); ?>
		$('tr').dblclick( function(){
<?php /* window.location.href = '/<?= $prefix ?>/features/<?= $config['index_db_click_action'] ?>/'+$(this).attr('row-id'); */ ?>
...
<?php if(isset($controller)){ ?>
		$(".pagination a[href^='<?= $base ?>/<?= $prefix ?>/features?sort=']").each(function(){ 
			this.href = this.href.replace("<?= $base ?>/<?= $prefix ?>/features?sort=", "<?= $base ?>/<?= $prefix ?>/features?page=1&sort=");
		});
		$(".pagination a[href='<?= $base ?>/<?= $prefix ?>/features']").each(function(){ 
			this.href = this.href.replace("<?= $base ?>/<?= $prefix ?>/features", "<?= $base ?>/<?= $prefix ?>/features?page=1");
		});
<?php } ?>
